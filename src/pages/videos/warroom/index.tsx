...
export default function WarRoom({ videos, error }: WarRoomProps) {
  return (
    <ErrorBoundary fallback={<div className="container mx-auto px-4 py-12"><h1>Error loading page.</h1></div>}>
      {error ? (
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-8">War Room Videos</h1>
        ...
        <div className="container mx-auto px-4 py-12">
          <Head>
            <title>War Room Videos | Rumble Video Site</title>
            <meta name="description" content="Watch the latest War Room videos on Rumble." />
            <meta name="keywords" content="War Room, videos, news" />
...
export const getServerSideProps: GetServerSideProps<WarRoomProps> = async () => {
  try {
    ...
    // Filter for War Room videos
    const videos = items
      .filter((item): item is Video => 
        item !== null && 
        item.type === 'video' &&
        item.uploader === 'War Room'
      )
      .sort((a, b) => {
